...
function quotedEq(column, value) {
  return `${column}=eq.${encodeURIComponent(`"${value}"`)}`;
}

function quotedIlike(column, value) {
...
async function requestOtp(email) {
  ...
  const recent = await rest(
    `client_otps?${quotedEq('email', normalized)}&order=created_at.desc&limit=1`,
...
async function verifyOtp(email, code) {
  ...
  const rows = await rest(
    `client_otps?${quotedEq('email', normalized)}&order=created_at.desc&limit=5`,
    { headers: { Prefer: 'return=representation' } }
  );
  const now = Date.now();
  ...
  await fetch(
    `${supabaseUrl()}/rest/v1/client_otps?${quotedEq('email', normalized)}`,
...
async function sessionFromToken(token) {
  ...
  const rows = await rest(
    `client_sessions?${quotedEq('token_hash', hashValue(token))}&limit=1`,
    { headers: { Prefer: 'return=representation' } }
  );
...
async function leadsForEmail(email) {
  const normalized = normalizeEmail(email);
  const rows = await rest(
    `leads?${quotedIlike('email', normalized)}&select=*&order=created_at.desc&limit=100`,
    { headers: { Prefer: 'return=representation' } }
  );
  return (rows || []).map((row) => ({
    id: row.id,
