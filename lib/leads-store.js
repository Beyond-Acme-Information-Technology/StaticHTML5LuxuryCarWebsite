...
function writeLocal(leads) {
  try {
    ...
    console.error('Local lead write skipped:', err.message);
  }
}
...
const HIDDEN_LEAD_TYPES = ['chauffeur_roster', 'chauffeur', 'chauffeur_session'];

function visibleLeads(leads) {
  return (leads || []).filter((lead) => !HIDDEN_LEAD_TYPES.includes(lead.type));
}
